...
export const addInscription = async (req, res) => {
  try {
    ...
    <body>
      <div class="email-container">
        <div class="content">
          ...
          <p>Dear ${savedInscription.firstName + " " + savedInscription.lastName},</p>
          <div class="login-details" style="width: 90%;">
          <p>Thank you for signing up. Your request for registration has been received.</p>
          <p>Please wait for approval from the admin. We will notify you via email once your account has been approved.</p>
    ...
    await sendEmail(savedInscription.email, "Welcome to Elkindy", body);
...
export const removeInscription = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Inscription deleted successfully." });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ success: false, error: err.message });
  }
...
export const approveInscription = async (req, res) => {
  try {
    ...
    const newUser = new User({
      ...
      verified: true,
      // Add additional fields from the inscription as needed
